#!/bin/sh
# Name: Autoback Dashboard
# Author: Angad Behl


# ---------------------------- EDIT BELOW THIS LINE ----------------------------
BASE_URL="http://localhost:5173" # Change if your server is running on a different URL or port
DRIVE_ID="FIND THE DRIVE ID FROM THE WEB UI (/drives) AND REPLACE THIS STRING"
API_KEY="CREATE AN API KEY AND REPLACE THIS STRING"
# ---------------------------- EDIT ABOVE THIS LINE ----------------------------

# Endpoint with JSON response
url="$BASE_URL/drives/$DRIVE_ID/backup"

fmt_secs() {
    secs=$1
    d=$((secs/86400))
    h=$(((secs%86400)/3600))
    m=$(((secs%3600)/60))
    s=$((secs%60))
    echo "${d}d ${h}h ${m}m ${s}s"
}

printf '\033[?1049h'                        # enter alternate screen
trap 'printf "\033[?1049l"' 0               # restore normal screen on exit

while true; do
printf '\033[H'

{
# Fetch the data
response=$(curl -s -X GET "$url" -H "x-api-key: $API_KEY")
# Response shape: { mounted: boolean, progress: ResticEvent | null }
# export type ResticEvent =
# 	| { message_type: 'status';  percent_done: number; total_files: number; files_done: number; total_bytes: number; bytes_done: number }
# 	| { message_type: 'summary'; files_new: number; files_changed: number; data_added: number; [key: string]: unknown }
# 	| { message_type: 'error';   error: string };

# Use jq to parse JSON data
mounted=$(echo "$response" | jq -r '.mounted')
message_type=$(echo "$response" | jq -r '.progress.message_type // empty')

if [ "$mounted" = "true" ]; then
  echo "Drive: plugged in"
else
  echo "Drive: unplugged"
fi

# https://www.digitalocean.com/community/tutorials/if-else-in-shell-scripts
if [ "$message_type" = "status" ]; then
# {"message_type":"status","seconds_elapsed":8,"seconds_remaining":31,"percent_done":0.21547865648915082,"total_files":4,"files_done":3,"total_bytes":2147483693,"bytes_done":462736901,"current_files":[".../autoback/test-data/to-backup/large-file.bin"]}
  percent_done=$(echo "$response" | jq -r '.progress.percent_done')
  percent_formatted=$(echo "$percent_done" | awk '{printf "%.1f%%\n", $1 * 100}')
  echo "Backup Progress: ${percent_formatted}"
  echo "Files Done: $(echo "$response" | jq -r '.progress.files_done // 0') / $(echo "$response" | jq -r '.progress.total_files // 0')"
  echo "Data Done: $(echo "$response" | jq -r '.progress.bytes_done // 0' | numfmt --to=iec) / $(echo "$response" | jq -r '.progress.total_bytes // 0' | numfmt --to=iec)"
  echo "Time Remaining: $(fmt_secs "$(echo "$response" | jq -r '.progress.seconds_remaining // 0')")"
elif [ "$message_type" = "error" ]; then
  error_message=$(echo "$response" | jq -r '.progress.error')
  echo "Error: $error_message"
elif [ "$message_type" = "summary" ]; then
  echo "Backup complete."
#   Full Response: {"message_type":"summary","files_new":4,"files_changed":0,"files_unmodified":0,"dirs_new":8,"dirs_changed":0,"dirs_unmodified":0,"data_blobs":4,"tree_blobs":9,"data_added":1902286,"data_added_packed":4426,"total_files_processed":4,"total_bytes_processed":10737418285,"total_duration":5.484627681,"backup_start":"2026-03-08T00:14:00.004346897-06:00","backup_end":"2026-03-08T00:14:05.488974548-06:00","snapshot_id":"6631ff825e6a3abb914d0570ae4395757f95a5c7d94a7b19a8736be12f54d1f7"}
  echo "Files New: $(echo "$response" | jq -r '.progress.files_new')"
  echo "Data Added: $(echo "$response" | jq -r '.progress.data_added' | numfmt --to=iec)"
#   echo "Full Response: $response"
else
  echo "No backup in progress."
fi

echo "-----------------------------------"
echo "Raw JSON Response:"
echo "$response"
} | while IFS= read -r _line; do printf '%s\033[K\n' "$_line"; done

printf '\033[J'
sleep 3
done
